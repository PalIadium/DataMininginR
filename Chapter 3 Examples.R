housing.df <- read.csv("~/R/Datasets/BostonHousing.csv")
head(housing.df, 9)


## line chart for the Amtrak data
Amtrak.df <- read.csv("~/R/Datasets/Amtrak.csv")

# A fix on margin error
# Should be getting [1] 5.1 4.1 4.1 2.1
par("mar")
# The correction
par(mar=c(1,1,1,1))
...
# Time series line graphs using different aggregation
library(forecast)
Amtrak.df <- read.csv("~/R/Datasets/Amtrak data.csv")
ridership.ts <- ts(Amtrak.df$Ridership, start = c(1991, 1), end = c(2004, 3), freq = 12)
## fit curve
...
utilities.df <- read.csv("~/R/Datasets/Utilities.csv")
...
# parallel coordinates plot for boston housing area
library(MASS)
par(mfcol = c(2,1))
...
#network graph
library(igraph)
ebay.df <- read.csv("~/R/Datasets/eBayNetwork.csv")
...
# Treemap
library(treemap)
tree.df <- read.csv("~/R/Datasets/EbayTreemap.csv")
...
# red dots in world map
library(ggmap)
SCstudents <- read.csv("~/R/Datasets/SC-US-students-GPS-data-2016.csv")
...
# shading world map
library(mosaic)
gdp.df <- read.csv("~/R/Datasets/gdp.csv", skip = 4, stringsAsFactors = FALSE)
names(gdp.df)[5] <- "GDP2015"
happiness.df <- read.csv("~/R/Datasets/Veerhoven.csv")
# gdp map
